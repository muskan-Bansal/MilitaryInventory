...
app.post('/register', checkNotAuthenticated, async (req, res) => {
  try {
    ...
    users.push(user);
    console.log('regisered', users);

    // let sql = `INSERT INTO admin(id,inventory_name,owner_name,email,password)VALUES( '${user}') `;
...
//     if (!err) res.resner({ employees: rows });
//     else console.log(err);
//   });
// });
...
// employee_id,employee_name,email,Role,Salary,joining_date
//Add employees
app.get('/addEmployees', checkAuthenticated, (req, res) => {
  let sql = 'SELECT * FROM employees ';
...
app.post('/submitEmployee', checkAuthenticated, (req, res) => {
  ...
  var file = req.files.img_url; //fileUploaded
  if (
    ...
    var insertBody =
      'INSERT INTO employees(employee_id,employee_name,email,Role,Salary,Joining_date,img_url)VALUES(?,?,?,?,?,?,?)';
...
app.post('/addcategory', checkAuthenticated, (req, res) => {
  let sql = `INSERT INTO categorydb(Category,userId) VALUES ('${req.body.new}','1671714543455') `;
  let query = mysqlConnection.query(sql, (err, rows, fields) => {
    if (!err) {
...
app.post('/addbrand', checkAuthenticated, (req, res) => {
  let sql = `INSERT INTO branddb(Brand,userId) VALUES ('${req.body.new}','1671714543455') `;
  let query = mysqlConnection.query(sql, (err, rows, fields) => {
    if (!err) {
      res.redirect('/brands');
    } else console.log(err);
...
app.post('/addsize', checkAuthenticated, (req, res) => {
  let sql = `INSERT INTO sizedb(Size,userId) VALUES ('${req.body.new}','1671714543455') `;
  let query = mysqlConnection.query(sql, (err, rows, fields) => {
    if (!err) {
      res.redirect('/sizes');
    } else console.log(err);
...
app.post('/stock_filter_query', checkAuthenticated, (req, res) => {
  ...
  if (filter_type == 'brand') {
    let sql =
      'SELECT Brand,img_url,count(*) AS Count,SUM(Amount) AS Amount FROM stockdb GROUP BY Brand';
    let query = mysqlConnection.query(sql, (err, rows, fields) => {
      if (!err) {
  ...
  if (filter_type == 'category') {
    let sql =
      'SELECT Category,img_url,count(*) AS Count,SUM(Amount) AS Amount FROM stockdb GROUP BY Category';
    let query = mysqlConnection.query(sql, (err, rows, fields) => {
      if (!err) {
        let sql1 = 'SELECT count(*) AS Count FROM stockdb';
        let query1 = mysqlConnection.query(sql1, (err1, rows1, fields1) => {
...
app.post('/submitstock', checkAuthenticated, (req, res) => {
  ...
  let query = mysqlConnection.query(sql, [data], (err, rows, fields) => {
    if (!err) {
      let sql2 = `update stockdb set img_url= ? where  itemID=  ? `;
      let query2 = mysqlConnection.query(
        sql2,
        [file.name, req.body.itemid1],
        (err, rows2, fields) => {
